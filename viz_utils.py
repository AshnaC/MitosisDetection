...
def filter_files(files, annotation_data, categories =[1,2]):
    valid_files =[]
    for file_name in files:
        image_id = image_filename2id(file_name)
...
def get_image_and_bboxes(image_filename, annotation_data,  region=None, level=0, categories: list=[1, 2], slide_folder=''):

    slide = openslide.open_slide(os.path.join(slide_folder, image_filename))
    level = 0
